...
logic        mem_stall;         // MEMORY STALL SIGNAL

logic [1:0] rom_mux;
logic [31:0] instr_prev;
...
logic        mem_r_id;          // (ID) ENABLE READ TO MEM
...
logic        mem_r_ex;          // (EX) ENABLE READ TO MEM
...
logic [31:0] rs2_data_mem1;      // (MEM) CONTENT OF SOURCE 2    COMMENTED FOR NOW!!!
logic [31:0] data_to_write_mem1;
...
logic        mem_r_mem1;          // (MEM) ENABLE READ TO MEM
logic        mem_w_mem_out1;         // (MEM) ENABLE WRITE TO MEM
...
logic [31:0] dram_addr_mem1;     // (MEM) Address to be read by ram
...
logic [31:0] r_data_raw_mem1;        // (MEM) DATA READ FROM MEM
...
logic        mem_r_mem2;         // (MEM) ENABLE READ TO MEM
...
logic [31:0] pc_wb;             // (WB) STAGE PC    COMMENTED FOR NOW!!
...
// Mem stall makes other stages stall but not vice versa
// always_comb begin
//     if (mem_stall)begin
...
ram ram(
    .clk(clk_divided),
    .w_addr(dram_addr_mem1),
    .w_data(data_to_write_mem1),
    .r_addr(dram_addr_mem1), // for now
    .r_data(r_data_raw_mem1), 
    .write_en(mem_w_mem_out1),
    //.read_en(mem_r_mem1)
    .read_en(1'b1)
);
...
rom rom(
    ...
    .rom_addr(rom_addr),
    .instr(rom_instr),
    .read_en(1'b1) // for now
);
// make a rom mux

// need to keep prevouis instr
always_ff @(posedge clk_divided) begin
    instr_prev<=instr_id;
...
id_ex_reg id_ex_reg(
    ...
    .mem_w_id(mem_w_id),
    .mem_r_id(mem_r_id),
    .set_mask_id(set_mask_id),

    // EX STAGE
    .set_mask_ex(set_mask_ex),
    .flush_ex(flush_ex),
    .pc_ex (pc_ex),
...
// WHEN MASKED WRITE : WE STALL FOR READ (READ STALL)
// WRITE ENABLE MUST STAY 1

// Step by step

// Store byte:
// 1) Read the word where the byte will be stored
...
mem_controller mem_controller (
        ...
        .funct3(funct3_mem1),
        .mem_data(r_data_raw_mem1), // we take the raw output from mem
        .store_data(data_to_write_mem1),
        .store_data_in(rs2_data_mem1), // ok
        .addr_lsb  (alu_result_mem1[1:0]),
        .opcode(opcode_mem1),
        .write_en_in(mem_w_mem1),
        .read_en(1'b1),
        .write_en_out(mem_w_mem_out1),
        .addr_in(alu_result_mem1),
        .dram_addr(dram_addr_mem1),
        .rom_addr(rom_addr),
        .set_mask(set_mask_mem1),
        //.rom_instr(rom_instr),
        //.instr(instr_id),
        .read_data(r_data_mem1), // output
        .mem_stall(mem_stall)
);
...
mem1_mem2_reg mem1_mem2_reg(
    ...
    .opcode_mem1 (opcode_mem1),
    .funct3_mem1 (funct3_mem1),
    .mem_w_mem1 (mem_w_mem_out1), // uses the modified value from the mem ctrl
    .mem_r_mem1 (mem_r_mem1),
    .wb_sel_mem1 (wb_sel_mem1),
    .reg_write_mem1 (reg_write_mem1),
    .csr_write_mem1 (csr_write_mem1),
